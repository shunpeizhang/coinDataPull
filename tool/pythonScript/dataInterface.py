...
def getSomeData(mysqlConn, start, count):
    ...
    sql = "SELECT Open, High, Low, Close, ID FROM (SELECT ID, Amount, COUNT, OPEN, CLOSE, Low, High, Vol FROM coin_data." \
          "`tb_coinData_30min` WHERE ID <= %d AND coinType = 0 ORDER BY id DESC LIMIT 100) info ORDER BY ID" % (1532664000 + 1800 * 50)
